...
from bevdepth.models.dino_bev_depth import DINOBEVDepth
...
class BEVDepthLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        self.model = DINOBEVDepth(self.backbone_conf, 
                                  self.head_conf,
        ...
        self.only_bev = only_bev

    def forward(self, sweep_imgs, lidar_depth, mats, img_metas, only_bev=False, given_bev=None, dino_out=None):
        return self.model(sweep_imgs, lidar_depth, mats, img_metas, only_bev=False, given_bev=None, dino_out=None)
    
    def training_step(self, batch):
        (sweep_imgs, mats, _, img_metas, gt_boxes, gt_labels, depth_labels) = batch    # depth_labels: [1, B, N, H, W]
        if len(depth_labels.shape) == 5:
            lidar_depth = depth_labels[:, 0, ...].cuda() # [1, 6, 490, 812]  
            # lidar_depth = self.get_downsampled_gt_depth(depth_labels)
        ...
        if self.only_bev:
            bev_feature = self(sweep_imgs, lidar_depth, mats, img_metas)
            return bev_feature
        else:
            preds, depth_preds = self(sweep_imgs, lidar_depth, mats, img_metas)
