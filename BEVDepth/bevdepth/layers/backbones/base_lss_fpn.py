...
class DepthNet(nn.Module):
    ...
    def forward(self, x, mats_dict):
        ...
        return torch.cat([depth, context], dim=1) # context: [6, 80, 16, 44]
...
class BaseLSSFPN(nn.Module):
    ...
    def _forward_single_sweep(self,
        ...
        depth_feature = self._forward_depth_net(     # [6, 192, 16, 44]
        ...
        if self.training or self.use_da:    
            img_feat_with_depth = depth.unsqueeze(                       # depth * context (depth:[6, 112, 16, 44], context:[6, 80, 16, 44])
                1) * depth_feature[:, self.depth_channels:(
                    self.depth_channels + self.output_channels)].unsqueeze(2)    # output_channels = 80, depth_channels = 112
